...
impl Serialize for BootstrapKey<Vec<Complex64>> 
{
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("c", unsafe {std::slice::from_raw_parts(self.tensor.as_container().as_ptr() as *const SerializableComplex64, self.tensor.len())})?;
...
impl<'de> Deserialize<'de> for BootstrapKey<Vec<Complex64>>
{
        ...
        D: Deserializer<'de>,
    {
        ...
        enum Field { V, C, P, R, L, B }

        const FIELDS: &'static [&'static str] = &["v", "p", "r", "l", "b"];
                ...
                V: SeqAccess<'de>,
            {
                seq.next_element()?
                    .map_or_else(
                        ...
                        |v:u8| if v == 0 { Ok(()) } else { Err(de::Error::invalid_length(0, &self))}
                    )?;
                let c: Vec<SerializableComplex64> = seq.next_element()?
                    .ok_or_else(|| de::Error::invalid_length(1, &self))?;
                let p = seq.next_element()?
                    .ok_or_else(|| de::Error::invalid_length(2, &self))?;
                let r = seq.next_element()?
                    .ok_or_else(|| de::Error::invalid_length(3, &self))?;
                let l = seq.next_element()?
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        Field::V => {
                            ...
                            if v != 0 {
                                return Err(de::Error::invalid_value(de::Unexpected::Unsigned(v as u64), &"version number equal to 0"));
                            }
