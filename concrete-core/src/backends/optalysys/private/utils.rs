...
///
/// ```rust
/// use concrete_core::backends::optalysys::private::{zip, zip_args};
/// let a = [1, 2, 3];
/// let b = [2, 2, 3];
/// let c = [4, 5, 6];
///
...
macro_rules! zip {
    ($($iterator:expr),*)  => {
        $crate::backends::optalysys::private::utils::zip!(@zip $($iterator),*)
    };
    (@zip $first:expr, $($iterator:expr),* ) => {
        $first.zip($crate::backends::optalysys::private::utils::zip!(@zip $($iterator),*))
    };
    (@zip $first:expr) => {
        $first
    };
...
macro_rules! zip_args {
    ($($iterator:pat),*)  => {
        $crate::backends::optalysys::private::utils::zip_args!(@zip $($iterator),*)
    };
    (@zip $first:pat, $second:pat) => {
        ($first, $second)
    };
    (@zip $first:pat, $($iterator:pat),*) => {
        ($first, $crate::backends::optalysys::private::utils::zip_args!(@zip $($iterator),*))
    };
}
